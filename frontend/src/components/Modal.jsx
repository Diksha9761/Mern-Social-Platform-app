            ...
            <div className="flex flex-col space-y-2 mt-2">
                 {
                    value && value.length>0? value.map((e,i)=> (
                        <Link className="bg-gray-500 py-2 px-3 text-white text-center rounded-md flex justify-center items-center gap-4"
                         to={`/user/${e._id}`} key={i} onClick={()=> setShow(false)}>
