        ...
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md">
            <div className="flex items-center space-x-2">
                ...
                to={`/user/${value.owner._id}`}>
                 <img src={value.owner.profilePic.url} 
                alt="" 
            ...
            <div className="mb-4">
                ...
                required
                />
                <button onClick={updateCaption} className="text-sm bg-blue-500 text-white px-1 py-1 rounded-md"
                disabled={captionLoading} 
                >
                {captionLoading? <LoadingAnimation/>:"Update Caption"} </button>
                <button  className="text-sm bg-red-500 text-white px-1 py-1 rounded-md" 
                onClick={()=>setShowInput(false)}>X</button>
...
export const Comment = ({ value, user, owner, id }) =>{
    ...
    return(
        <div className="flex items-center space-x-2 mt-2">
            ...
            <img src={value.user.profilePic.url} 
            className="w-8 h-8 rounded-full"
            alt="" 
            />
            </Link>
          ...
          {
            owner === user._id ? "" : <>
             {value.user._id === user._id && ( <button onClick={deleteCommentHandler} className="text-red-500">
                <MdDelete/>
            </button>)}
            </>            
          }
