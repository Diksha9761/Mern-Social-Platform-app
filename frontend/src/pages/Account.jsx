    ...
    const [followingsData,setFollowingsData] = useState([]);
      ...
      async function followData(){
        try {
            ...
            setFollowingsData(data.user.followings);
        } catch (error) {
            console.log(error);
        }  
    }
  ...
  return (
    ...
    <div className="bg-white flex justify-between gap-4 p-8 rounded-lg shadow-md max-w-md">
        <div className="image flex flex-col justify-between mb-4 gap-4">
            ...
            required
             /> 
            <button className="bg-blue-500 text-white px-3 py-2" onClick={changeImageHandler}>
              Update Profile
              </button>
            </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
           ...
           <div className="flex justify-center items-center gap-2">
            ...
            :
            <p className="text-gray-800 font-semibold">{user.name}<button onClick={()=>setShowInput(true)}>
              <CiEdit/>
              </button> 
            </p>}
            ...
            <p className="text-gray-500 text-sm cursor-pointer"
             ...
             >
            {user.followings.length}following</p>
            <button 
      ...
      <button onClick={()=>setShowUpdatePass(!showUpdatePass)} className="bg-blue-500 px-2 py-1 rounded-sm text-white">
        {showUpdatePass ? "X" : "Update Password"}
        </button>

        {showUpdatePass && <form onSubmit={updatePassword} className="flex justify-center items-center flex-col bg-white p-2 rounded-sm gap-4">
          <input type="password" 
    ...
    <div className="flex justify-center items-center bg-white p-4 rounded-md gap-7">
